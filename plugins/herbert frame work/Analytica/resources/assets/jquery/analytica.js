...
/* 	if ( typeof ajax_object === 'undefined' ){
	...
	jQuery('body').on(
		'click', '.prev-page,.next-page,.first-page,.last-page', function(event){
		  ...
		  if(page_number != 0){
			var url = '/test/wp-admin/admin.php?page=analytica-admin-settings&action=nextpage';
		    jQuery.post( url, request_input, function(data){
		    //console.log( data );
		   //jQuery.get(ajax_object.ajax_url, request_input, function(data){
				...
				if( page_number === parseInt(jQuery('#total-pages').html()) && posts.length < 10 ){
	...
	jQuery('#draft').click( function( event ){
		...
		jQuery('input[type="checkbox"]:checked').each(function(){
			...
			//console.log(jQuery('#' + jQuery(this).val() + '> strong > span').attr('class'));
			if( jQuery('#' + jQuery(this).val() + '> strong > span').attr('class') !== undefined ){
			    selectedPosts.push(jQuery('#' + jQuery(this).val() + '> strong > span').attr('class'));
			}
		});
		
		//changeStatus(selectedPosts);
		
		var data = {
			selected_posts: selectedPosts,
		};
		//console.log( data );
		var url = '/test/wp-admin/admin.php?page=analytica-admin-settings&action=status-change';
