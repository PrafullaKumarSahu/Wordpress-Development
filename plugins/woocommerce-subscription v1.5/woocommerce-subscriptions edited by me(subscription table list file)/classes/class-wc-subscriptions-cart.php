...
class WC_Subscriptions_Cart {
	...
	public static function calculate_subscription_totals( $total ) {
		...
		if ( 0 == $cart_sign_up_fee && $cart_has_trial ) {
			...
			// If we're not purchasing any products with the subscription, make sure the free trial is truly free (i.e. no fees)
			if ( 0 == $free_trial_cart->total ) {
		...
		$woocommerce->cart->recurring_shipping_total     = self::calculate_recurring_shipping();
		...
		if ( 0 == $cart_sign_up_fee && $cart_has_trial && 0 == $woocommerce->cart->total ) {
			foreach ( $woocommerce->cart->get_fees() as $fee_index => $fee ) {
		...
		$woocommerce->cart->recurring_total = $recurring_cart->total - $recurring_cart->shipping_total + $woocommerce->cart->recurring_shipping_total;
	...
	public static function set_calculated_total( $total ) {
		...
		if ( 'sign_up_fee_total' == self::$calculation_type || 'free_trial_total' == self::$calculation_type ) {
			...
			$total = $total - $woocommerce->cart->shipping_tax_total - $woocommerce->recurring_shipping_total;
	...
	public static function get_cart_shipping_method_full_label( $full_label, $method ) {
		...
		if ( self::cart_contains_subscription() && $method->cost > 0 && $woocommerce->cart->recurring_shipping_total == $woocommerce->cart->shipping_total ) {
			$full_label = self::get_cart_subscription_string( '', $full_label );
		} elseif ( self::cart_contains_subscription() && $method->cost > 0 && $woocommerce->cart->recurring_shipping_total != $woocommerce->cart->shipping_total ) {
			$full_label = self::get_cart_subscription_string( $woocommerce->cart->shipping_total, $full_label );
		}
	 ...
	 * is not a subscription and calculate the totals.
	 *
	 * @return string
	 *
	 * @since 1.5
	...
	public static function calculate_recurring_shipping() {
		...
		foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $values ) {
			if ( WC_Subscriptions_Product::is_subscription( $values['product_id'] ) ) {
				$packages[0]['contents'][ $cart_item_key ] = $values;				
			}
		}
